// Runs every .smt2 in cpp/tests/bool/ in both sequential and parallel modes,
// verifying the answer matches the {_sat, _unsat} filename convention.
//
// These tests exercise the SAT-driven path: each formula contains Boolean
// structure (or, and, =>, xor, ite, distinct) so it routes through
// sat_solve_timed and the IPASIR-UP propagator instead of the legacy
// top-level-conjunctive fast path.
...
int main() {
#ifndef BOOL_TESTS_DIR
#error "BOOL_TESTS_DIR must be defined"
#endif
  int seq_ok = 0, seq_total = 0;
  int par_ok = 0, par_total = 0;
  int failures = 0;
  for (const auto& path : list_smt2(BOOL_TESTS_DIR)) {
  ...
  std::printf("bool_regression_test: sequential=%d/%d parallel=%d/%d\n",
