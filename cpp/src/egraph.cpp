...
std::unique_ptr<EGraph> EGraph::clone() const {
  ...
  // Proof-tracking state. Empty in the default case; full copy when in
  // proof-tracking mode. Trivially copyable POD.
  out->merge_log_ = merge_log_;
  out->explain_adj_ = explain_adj_;
...
Id EGraph::add(ENode node) {
  ...
  // Make sure the explain adjacency vector covers the new class id so
  // that merge_with_reason can write into it without bounds-checking.
  // (Cheap: a vector resize on the rare path where a new class id is
  // larger than the current vector. Untouched by default merge/rebuild.)
  if (explain_adj_.size() <= id) explain_adj_.resize(id + 1);
...
// ===========================================================================
// Proof-tracking ("explain") implementations
// ===========================================================================
// These are entirely sequential and only touch the proof-tracking state
// (merge_log_, explain_adj_). The default merge / repair / rebuild paths
// above never call these — proof tracking is opt-in.
//
// The model:
//   merge_with_reason(a, b, r) appends an event (a, b, r) to merge_log_
//   AND adds an undirected edge labeled with the event's index to
//   explain_adj_[a] and explain_adj_[b]. It then delegates to the
//   existing merge(a, b) for UF / classes_ / parents_ / worklist_ updates.
//
//   rebuild_with_reasons() runs the same loop as rebuild(), but uses a
//   proof-tracking variant of repair() that calls merge_with_reason
//   with a Congruence reason whenever it discovers a hash-collapse.
//
//   explain(a, b) walks the adjacency graph (BFS), reconstructs the path
//   of MergeEvents from a to b, recursively expands any Congruence
//   reason into proofs of its children's equalities, then collapses the
//   resulting flat list of asserted SAT lits via a fresh UF to drop
//   redundant chains.
// ===========================================================================

Id EGraph::merge_with_reason(Id a, Id b, ProofReason r) {
  Id ra = find(a), rb = find(b);
  if (ra == rb) return ra;  // already equal — no log entry, nothing to do
  // Append an event for THIS merge. We record the (pre-find) endpoints
  // so explain() recursing on a child equality can ask
  // explain(children_a[i], children_b[i]) without worrying about how
  // the union-find has evolved.
  std::uint32_t event_idx = static_cast<std::uint32_t>(merge_log_.size());
  merge_log_.push_back({a, b, std::move(r)});
  // Make sure adjacency vector is large enough. add() pre-resizes for
  // newly minted class ids, but a caller passing in raw ids that have
  // not been seen by add() (shouldn't happen for normal use) would
  // need this safety net.
  Id maxid = a > b ? a : b;
  if (explain_adj_.size() <= maxid) explain_adj_.resize(maxid + 1);
  explain_adj_[a].emplace_back(b, event_idx);
  explain_adj_[b].emplace_back(a, event_idx);
  return merge(a, b);
}

namespace {
// Helper: scan the parent-list of a class for parent enodes that hash-
// collapse to the same canonical key.  Used inside repair_with_reasons
// to discover congruence merges with their witness children.
//
// We mirror the existing `repair`'s loop, but instead of doing
// hashcons-erase-reinsert silently, we track which old-parent's
// (canonicalized) form collided with an existing hashcons entry, and
// emit a Congruence reason carrying both the old and new sides'
// children lists (raw, pre-canonicalization-of-children).
}  // namespace

// rebuild_with_reasons replicates the existing sequential rebuild loop
// but routes congruence-merge discoveries through merge_with_reason.
// It uses an internal proof-tracking variant of repair() implemented
// inline below to avoid touching the default repair().
void EGraph::rebuild_with_reasons() {
  // Local helper: a proof-tracking repair pass over a single class.
    ...
    for (auto& entry : old_parents) {
      ...
      if (existing_it != hashcons_.end()) {
        ...
        if (find(p_id) != find(existing)) {
          // Build the Congruence reason. Both sides have the *same*
          // operator (otherwise the canonical form wouldn't match) and
          // the same arity. We carry the raw children of the
          // newly-canonicalized p_canon (= the children list at this
          // moment) and... we don't know the OTHER side's pre-canon
          // children (they were stored under the existing hashcons
          // entry but we only have the class id, not the original
          // enode). We do know: existing_it->first IS that ENode, and
          // its children are exactly the children we want.
          ProofReason r = ProofReason::congruence(
              existing_it->first.children, p_canon.children);
          p_id = merge_with_reason(p_id, existing, std::move(r));
        }
      }
...
std::vector<int> EGraph::explain(Id a, Id b) {
  // Walks the adjacency graph from a to b, reconstructs the merge path,
  // expands any Congruence reasons into proofs of their child equalities
  // (recursive), then dedupes the resulting flat list of asserted lits
  // via a fresh union-find.
  std::vector<int> result;
  if (a == b) return result;

  // Worklist of equalities still to explain. Each entry asks "give me
  // the asserted lits for why x ≡ y" — by walking the merge_log_ graph.
  std::vector<std::pair<Id, Id>> todo;
  todo.emplace_back(a, b);

  // Collect the raw asserted reasons (with their endpoints) here. Order
  // matters for the dedupe step at the end; we keep insertion order.
  ...
  while (!todo.empty()) {
    ...
    // BFS from x looking for y. Record predecessor for path reconstruction.
    std::unordered_map<Id, std::pair<Id, std::uint32_t>> pred;
    pred.emplace(x, std::make_pair(x, ~std::uint32_t(0)));  // sentinel
    std::deque<Id> q;
    q.push_back(x);
    bool found = false;
    while (!q.empty()) {
      Id cur = q.front(); q.pop_front();
      if (cur == y) { found = true; break; }
      if (cur >= explain_adj_.size()) continue;
      for (auto [nbr, evt] : explain_adj_[cur]) {
    ...
    if (!found) continue;  // shouldn't happen if a ≡ b in UF — defensive

    // Walk the path back from y to x, processing each event's reason.
    Id cur = y;
    while (cur != x) {
      auto& [prev, evt] = pred[cur];
      const MergeEvent& ev = merge_log_[evt];
      if (ev.reason.kind == ProofReason::Kind::Asserted) {
        hits.push_back({ev.reason.sat_lit, ev.a, ev.b});
      } else {
        // Congruence: recursively explain each child pair.
  ...
  // ----- dedupe via fresh UF -----------------------------------------------
  // Spin up a fresh UF over our class id space. For each hit in order,
  // ask whether its two endpoints are already equivalent in the fresh
  // UF. If they are, the hit is redundant — skip. Otherwise union them
  // and keep the lit. The kept set forms a spanning forest of the
  // conflict's relevant component.
